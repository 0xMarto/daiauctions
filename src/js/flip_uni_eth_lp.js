...
const UNI_ETH_LP_FLIP_ADDRESS = "0xe5ED7da0483e291485011D5372F3BF46235EB277";
const MEDIANIZER_ADDRESS = "0x8Ce9E9442F2791FC63CD6394cC12F2dE4fbc1D71";
const CAT_ADDRESS = "0xa5679C04fc3d9d8b0AaB1F0ab83555b301cA70Ea";
const FLIP_ILK = "0x554e495632554e494554482d4100000000000000000000000000000000000000";
...
const flipContract = new web3.eth.Contract(FLIPPER_ABI, UNI_ETH_LP_FLIP_ADDRESS);
...
async function showEvents(someID) {
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        if (event.event === "Kick") {
            ...
            values += "lot: " + lot.toFixed(3) + " uni-eth-lp | ";
            ...
            if (!medPrice) {
                values += "Paid Rate: $" + auctions[flipId]["paidPrice"] + " dai/uni-eth-lp (+-.--%) | ";
                values += "--,-- % | Price: $---,-- | ";
            } else {
                values += "Paid Rate: $" + auctions[flipId]["paidPrice"] + " dai/uni-eth-lp ";
...
function showAuctionDetails(id) {
    ...
    msg += `- LOT: ${auction.kickLot.toLocaleString('en', numFormat)} uni-eth-lp `;
    msg += `- BID: ${auction.bid.toLocaleString('en', numFormat)} dai <br/>`;
    msg += `- UNI-ETH LP MED: $${auction.kickPrice} uni-eth-lp/dai <br/><br/>`;
    ...
    msg += `- LOT: ${auction.lot.toLocaleString('en', numFormat)} uni-eth-lp `;
    msg += `- BID: ${auction.bid.toLocaleString('en', numFormat)} dai <br/>`;
    msg += `- PAID PRICE: $${auction.paidPrice} uni-eth-lp/dai <br/>`;
    msg += `- UNI-ETH LP MED: $${auction.bidPrice} uni-eth-lp/dai <br/><br/>`;

    msg += 'Ended: <br/>';
    msg += `- DATE: ${auction.dealDate} <br/>`;
    msg += `- FROM: ${auction.txFrom} <br/>`;
    msg += `- PAID PRICE: $${auction.paidPrice} uni-eth-lp/dai <br/>`;
    msg += `- UNI-ETH LP MED: $${auction.dealPrice} uni-eth-lp/dai <br/><br/>`;
