...
const LINK_FLIP_ADDRESS = "0xB907EEdD63a30A3381E6D898e5815Ee8c9fd2c85";
...
const FLIP_ILK = "0x4c494e4b2d410000000000000000000000000000000000000000000000000000";
...
const flipContract = new web3.eth.Contract(FLIPPER_ABI, LINK_FLIP_ADDRESS);
...
var showEvents = async function showEvents(someID) {
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        if (event.event === "Kick") {
            ...
            if (!medPrice) {
                values += "Paid Rate: $" + auctions[flipId]["paidPrice"] + " dai/link (+-.--%) | ";
                values += "--,-- % | Price: $---,-- | ";
            } else {
                values += "Paid Rate: $" + auctions[flipId]["paidPrice"] + " dai/link ";
...
function showAuctionDetails(id){
    ...
    msg += `- LOT: ${auction.kickLot.toLocaleString('en', numFormat)} link `;
    msg += `- BID: ${auction.bid.toLocaleString('en', numFormat)} dai <br/>`;
    msg += `- LINK MED: $${auction.kickPrice} link/dai <br/><br/>`;
    msg += `Bids received: ${auction.tends + auction.dents} <br/><br/>`;

    msg += 'Last Bid:<br/>';
    msg += `- DATE: ${auction.bidDate} <br/>`;
    msg += `- FROM: ${auction.bidFrom} <br/>`;
    msg += `- LOT: ${auction.lot.toLocaleString('en', numFormat)} link `;
    msg += `- BID: ${auction.bid.toLocaleString('en', numFormat)} dai <br/>`;
    msg += `- PAID PRICE: $${auction.paidPrice} link/dai <br/>`;
    msg += `- LINK MED: $${auction.bidPrice} link/dai <br/><br/>`;

    msg += 'Ended: <br/>';
    msg += `- DATE: ${auction.dealDate} <br/>`;
    msg += `- FROM: ${auction.txFrom} <br/>`;
    msg += `- PAID PRICE: $${auction.paidPrice} link/dai <br/>`;
    msg += `- LINK MED: $${auction.dealPrice} link/dai <br/><br/>`;
