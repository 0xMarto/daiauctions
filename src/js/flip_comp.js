...
const COMP_FLIP_ADDRESS = "0x524826F84cB3A19B6593370a5889A58c00554739";
const MEDIANIZER_ADDRESS = "0xA3421Be733125405Ea20aA853839D34b364eB524";
const CAT_ADDRESS = "0xa5679C04fc3d9d8b0AaB1F0ab83555b301cA70Ea";
const FLIP_ILK = "0x434f4d502d410000000000000000000000000000000000000000000000000000";
...
const flipContract = new web3.eth.Contract(FLIPPER_ABI, COMP_FLIP_ADDRESS);
...
async function showEvents(someID) {
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        if (event.event === "Kick") {
            ...
            if (!medPrice) {
                values += "Paid Rate: $" + auctions[flipId]["paidPrice"] + " dai/comp (+-.--%) | ";
                values += "--,-- % | Price: $---,-- | ";
            } else {
                values += "Paid Rate: $" + auctions[flipId]["paidPrice"] + " dai/comp ";
...
function showAuctionDetails(id) {
    ...
    msg += `- LOT: ${auction.kickLot.toLocaleString('en', numFormat)} comp `;
    msg += `- BID: ${auction.bid.toLocaleString('en', numFormat)} dai <br/>`;
    msg += `- COMP MED: $${auction.kickPrice} comp/dai <br/><br/>`;
    ...
    msg += `- LOT: ${auction.lot.toLocaleString('en', numFormat)} comp `;
    msg += `- BID: ${auction.bid.toLocaleString('en', numFormat)} dai <br/>`;
    msg += `- PAID PRICE: $${auction.paidPrice} comp/dai <br/>`;
    msg += `- COMP MED: $${auction.bidPrice} comp/dai <br/><br/>`;

    msg += 'Ended: <br/>';
    msg += `- DATE: ${auction.dealDate} <br/>`;
    msg += `- FROM: ${auction.txFrom} <br/>`;
    msg += `- PAID PRICE: $${auction.paidPrice} comp/dai <br/>`;
    msg += `- COMP MED: $${auction.dealPrice} comp/dai <br/><br/>`;
