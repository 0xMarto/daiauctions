...
const KNC_FLIP_ADDRESS = "0x57B01F1B3C59e2C0bdfF3EC9563B71EEc99a3f2f";
const MEDIANIZER_ADDRESS = "0x83076a2F42dc1925537165045c9FDe9A4B71AD97";
...
const flipContract = new web3.eth.Contract(FLIPPER_ABI, KNC_FLIP_ADDRESS);
...
var showEvents = async function showEvents(someID) {
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        if (event.event === "Kick") {
            ...
            if (!medPrice) {
                values += "Paid Rate: $" + auctions[flipId]["paidPrice"] + " dai/knc (+-.--%) | ";
                values += "--,-- % | Price: $---,-- | ";
            } else {
                values += "Paid Rate: $" + auctions[flipId]["paidPrice"] + " dai/knc ";
...
function showAuctionDetails(id){
    ...
    msg += `- LOT: ${auction.kickLot.toLocaleString('en', numFormat)} knc `;
    msg += `- BID: ${auction.bid.toLocaleString('en', numFormat)} dai <br/>`;
    msg += `- KNC MED: $${auction.kickPrice} knc/dai <br/><br/>`;
    msg += `Bids received: ${auction.tends + auction.dents} <br/><br/>`;

    msg += 'Last Bid:<br/>';
    msg += `- DATE: ${auction.bidDate} <br/>`;
    msg += `- FROM: ${auction.bidFrom} <br/>`;
    msg += `- LOT: ${auction.lot.toLocaleString('en', numFormat)} knc `;
    msg += `- BID: ${auction.bid.toLocaleString('en', numFormat)} dai <br/>`;
    msg += `- PAID PRICE: $${auction.paidPrice} knc/dai <br/>`;
    msg += `- KNC MED: $${auction.bidPrice} knc/dai <br/><br/>`;

    msg += 'Ended: <br/>';
    msg += `- DATE: ${auction.dealDate} <br/>`;
    msg += `- FROM: ${auction.txFrom} <br/>`;
    msg += `- PAID PRICE: $${auction.paidPrice} knc/dai <br/>`;
    msg += `- KNC MED: $${auction.dealPrice} knc/dai <br/><br/>`;
