...
const USDT_FLIP_ADDRESS = "0x667F41d0fDcE1945eE0f56A79dd6c142E37fCC26";
const MEDIANIZER_ADDRESS = "0x56D4bBF358D7790579b55eA6Af3f605BcA2c0C3A";
const CAT_ADDRESS = "0xa5679C04fc3d9d8b0AaB1F0ab83555b301cA70Ea";
const FLIP_ILK = "0x555344542d410000000000000000000000000000000000000000000000000000";
...
const flipContract = new web3.eth.Contract(FLIPPER_ABI, USDT_FLIP_ADDRESS);
...
var showEvents = async function showEvents(someID) {
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        if (event.event === "Kick") {
            ...
            if (!medPrice) {
                values += "Paid Rate: $" + auctions[flipId]["paidPrice"] + " dai/usdt (+-.--%) | ";
                values += "--,-- % | Price: $---,-- | ";
            } else {
                values += "Paid Rate: $" + auctions[flipId]["paidPrice"] + " dai/usdt ";
...
function showAuctionDetails(id){
    ...
    msg += `- LOT: ${auction.kickLot.toLocaleString('en', numFormat)} usdt `;
    msg += `- BID: ${auction.bid.toLocaleString('en', numFormat)} dai <br/>`;
    msg += `- USDT MED: $${auction.kickPrice} usdt/dai <br/><br/>`;
    msg += `Bids received: ${auction.tends + auction.dents} <br/><br/>`;

    msg += 'Last Bid:<br/>';
    msg += `- DATE: ${auction.bidDate} <br/>`;
    msg += `- FROM: ${auction.bidFrom} <br/>`;
    msg += `- LOT: ${auction.lot.toLocaleString('en', numFormat)} usdt `;
    msg += `- BID: ${auction.bid.toLocaleString('en', numFormat)} dai <br/>`;
    msg += `- PAID PRICE: $${auction.paidPrice} usdt/dai <br/>`;
    msg += `- USDT MED: $${auction.bidPrice} usdt/dai <br/><br/>`;

    msg += 'Ended: <br/>';
    msg += `- DATE: ${auction.dealDate} <br/>`;
    msg += `- FROM: ${auction.txFrom} <br/>`;
    msg += `- PAID PRICE: $${auction.paidPrice} usdt/dai <br/>`;
    msg += `- USDT MED: $${auction.dealPrice} usdt/dai <br/><br/>`;
