...
const RENBTC_FLIP_ADDRESS = "0x30BC6eBC27372e50606880a36B279240c0bA0758";
const MEDIANIZER_ADDRESS = "0xe0F30cb149fAADC7247E953746Be9BbBB6B5751f";
const CAT_ADDRESS = "0xa5679C04fc3d9d8b0AaB1F0ab83555b301cA70Ea";
const FLIP_ILK = "0x52454e4254432d41000000000000000000000000000000000000000000000000";
...
const flipContract = new web3.eth.Contract(FLIPPER_ABI, RENBTC_FLIP_ADDRESS);
...
async function showEvents(someID) {
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        if (event.event === "Kick") {
            ...
            values += "lot: " + lot.toFixed(3) + " renbtc | ";
            ...
            if (!medPrice) {
                values += "Paid Rate: $" + auctions[flipId]["paidPrice"] + " dai/renbtc (+-.--%) | ";
                values += "--,-- % | Price: $---,-- | ";
            } else {
                values += "Paid Rate: $" + auctions[flipId]["paidPrice"] + " dai/renbtc ";
...
async function updateGlobals() {
    ...
    let numFormat = {minimumFractionDigits: 2, maximumFractionDigits: 2};
    globalsPanel.find('#price-btc').text("$" + medPrice.toLocaleString('en', numFormat));
}
...
function showAuctionDetails(id){
    ...
    msg += `- LOT: ${auction.kickLot.toLocaleString('en', numFormat)} btc `;
    msg += `- BID: ${auction.bid.toLocaleString('en', numFormat)} dai <br/>`;
    msg += `- RENBTC MED: $${auction.kickPrice} btc/dai <br/><br/>`;
    ...
    msg += `- LOT: ${auction.lot.toLocaleString('en', numFormat)} btc `;
    msg += `- BID: ${auction.bid.toLocaleString('en', numFormat)} dai <br/>`;
    msg += `- PAID PRICE: $${auction.paidPrice} btc/dai <br/>`;
    msg += `- RENBTC MED: $${auction.bidPrice} btc/dai <br/><br/>`;

    msg += 'Ended: <br/>';
    msg += `- DATE: ${auction.dealDate} <br/>`;
    msg += `- FROM: ${auction.txFrom} <br/>`;
    msg += `- PAID PRICE: $${auction.paidPrice} btc/dai <br/>`;
    msg += `- RENBTC MED: $${auction.dealPrice} btc/dai <br/><br/>`;
