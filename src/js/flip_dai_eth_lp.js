...
async function showEvents(someID) {
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        if (event.event === "Kick") {
            ...
            values += "lot: " + lot.toFixed(3) + " dai-eth-lp | ";
            ...
            if (!medPrice) {
                values += "Paid Rate: $" + auctions[flipId]["paidPrice"] + " dai/dai-eth-lp (+-.--%) | ";
                values += "--,-- % | Price: $---,-- | ";
            } else {
                values += "Paid Rate: $" + auctions[flipId]["paidPrice"] + " dai/dai-eth-lp ";
...
function showAuctionDetails(id) {
    ...
    msg += `- LOT: ${auction.kickLot.toLocaleString('en', numFormat)} dai-eth-lp `;
    msg += `- BID: ${auction.bid.toLocaleString('en', numFormat)} dai <br/>`;
    msg += `- DAI-ETH LP MED: $${auction.kickPrice} dai-eth-lp/dai <br/><br/>`;
    ...
    msg += `- LOT: ${auction.lot.toLocaleString('en', numFormat)} dai-eth-lp `;
    msg += `- BID: ${auction.bid.toLocaleString('en', numFormat)} dai <br/>`;
    msg += `- PAID PRICE: $${auction.paidPrice} dai-eth-lp/dai <br/>`;
    msg += `- DAI-ETH LP MED: $${auction.bidPrice} dai-eth-lp/dai <br/><br/>`;

    msg += 'Ended: <br/>';
    msg += `- DATE: ${auction.dealDate} <br/>`;
    msg += `- FROM: ${auction.txFrom} <br/>`;
    msg += `- PAID PRICE: $${auction.paidPrice} dai-eth-lp/dai <br/>`;
    msg += `- DAI-ETH LP MED: $${auction.dealPrice} dai-eth-lp/dai <br/><br/>`;
