...
const YFI_FLIP_ADDRESS = "0xEe4C9C36257afB8098059a4763A374a4ECFE28A7";
const MEDIANIZER_ADDRESS = "0x89AC26C0aFCB28EC55B6CD2F6b7DAD867Fa24639";
const CAT_ADDRESS = "0xa5679C04fc3d9d8b0AaB1F0ab83555b301cA70Ea";
const FLIP_ILK = "0x5946492d41000000000000000000000000000000000000000000000000000000";
...
const flipContract = new web3.eth.Contract(FLIPPER_ABI, YFI_FLIP_ADDRESS);
...
async function showEvents(someID) {
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        if (event.event === "Kick") {
            ...
            if (!medPrice) {
                values += "Paid Rate: $" + auctions[flipId]["paidPrice"] + " dai/yfi (+-.--%) | ";
                values += "--,-- % | Price: $---,-- | ";
            } else {
                values += "Paid Rate: $" + auctions[flipId]["paidPrice"] + " dai/yfi ";
...
async function updateGlobals() {
    ...
    let numFormat = {minimumFractionDigits: 2, maximumFractionDigits: 2};
    globalsPanel.find('#price-yfi').text("$" + medPrice.toLocaleString('en', numFormat));
}
...
function showAuctionDetails(id){
    ...
    msg += `- LOT: ${auction.kickLot.toLocaleString('en', numFormat)} yfi `;
    msg += `- BID: ${auction.bid.toLocaleString('en', numFormat)} dai <br/>`;
    msg += `- YFI MED: $${auction.kickPrice} yfi/dai <br/><br/>`;
    ...
    msg += `- LOT: ${auction.lot.toLocaleString('en', numFormat)} yfi `;
    msg += `- BID: ${auction.bid.toLocaleString('en', numFormat)} dai <br/>`;
    msg += `- PAID PRICE: $${auction.paidPrice} yfi/dai <br/>`;
    msg += `- YFI MED: $${auction.bidPrice} yfi/dai <br/><br/>`;

    msg += 'Ended: <br/>';
    msg += `- DATE: ${auction.dealDate} <br/>`;
    msg += `- FROM: ${auction.txFrom} <br/>`;
    msg += `- PAID PRICE: $${auction.paidPrice} yfi/dai <br/>`;
    msg += `- YFI MED: $${auction.dealPrice} yfi/dai <br/><br/>`;
