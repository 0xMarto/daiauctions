...
const AAVE_FLIP_ADDRESS = "0x16e1b844094c885a37509a8f76c533B5fbFED13a";
const MEDIANIZER_ADDRESS = "0xe62872DFEbd323b03D27946f8e2491B454a69811";
const CAT_ADDRESS = "0xa5679C04fc3d9d8b0AaB1F0ab83555b301cA70Ea";
const FLIP_ILK = "0x414156452d410000000000000000000000000000000000000000000000000000";
...
const flipContract = new web3.eth.Contract(FLIPPER_ABI, AAVE_FLIP_ADDRESS);
...
async function showEvents(someID) {
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        if (event.event === "Kick") {
            ...
            if (!medPrice) {
                values += "Paid Rate: $" + auctions[flipId]["paidPrice"] + " dai/aave (+-.--%) | ";
                values += "--,-- % | Price: $---,-- | ";
            } else {
                values += "Paid Rate: $" + auctions[flipId]["paidPrice"] + " dai/aave ";
...
function showAuctionDetails(id) {
    ...
    msg += `- LOT: ${auction.kickLot.toLocaleString('en', numFormat)} aave `;
    msg += `- BID: ${auction.bid.toLocaleString('en', numFormat)} dai <br/>`;
    msg += `- AAVE MED: $${auction.kickPrice} aave/dai <br/><br/>`;
    ...
    msg += `- LOT: ${auction.lot.toLocaleString('en', numFormat)} aave `;
    msg += `- BID: ${auction.bid.toLocaleString('en', numFormat)} dai <br/>`;
    msg += `- PAID PRICE: $${auction.paidPrice} aave/dai <br/>`;
    msg += `- AAVE MED: $${auction.bidPrice} aave/dai <br/><br/>`;

    msg += 'Ended: <br/>';
    msg += `- DATE: ${auction.dealDate} <br/>`;
    msg += `- FROM: ${auction.txFrom} <br/>`;
    msg += `- PAID PRICE: $${auction.paidPrice} aave/dai <br/>`;
    msg += `- AAVE MED: $${auction.dealPrice} aave/dai <br/><br/>`;
