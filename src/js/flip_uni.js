...
const UNI_FLIP_ADDRESS = "0xF5b8cD9dB5a0EC031304A7B815010aa7761BD426";
const MEDIANIZER_ADDRESS = "0x52f761908cC27B4D77AD7A329463cf08baf62153";
const CAT_ADDRESS = "0xa5679C04fc3d9d8b0AaB1F0ab83555b301cA70Ea";
const FLIP_ILK = "0x554e492d41000000000000000000000000000000000000000000000000000000";
...
const flipContract = new web3.eth.Contract(FLIPPER_ABI, UNI_FLIP_ADDRESS);
...
var showEvents = async function showEvents(someID) {
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        if (event.event === "Kick") {
            ...
            if (!medPrice) {
                values += "Paid Rate: $" + auctions[flipId]["paidPrice"] + " dai/uni (+-.--%) | ";
                values += "--,-- % | Price: $---,-- | ";
            } else {
                values += "Paid Rate: $" + auctions[flipId]["paidPrice"] + " dai/uni ";
...
function showAuctionDetails(id){
    ...
    msg += `- LOT: ${auction.kickLot.toLocaleString('en', numFormat)} uni `;
    msg += `- BID: ${auction.bid.toLocaleString('en', numFormat)} dai <br/>`;
    msg += `- UNI MED: $${auction.kickPrice} uni/dai <br/><br/>`;
    msg += `Bids received: ${auction.tends + auction.dents} <br/><br/>`;

    msg += 'Last Bid:<br/>';
    msg += `- DATE: ${auction.bidDate} <br/>`;
    msg += `- FROM: ${auction.bidFrom} <br/>`;
    msg += `- LOT: ${auction.lot.toLocaleString('en', numFormat)} uni `;
    msg += `- BID: ${auction.bid.toLocaleString('en', numFormat)} dai <br/>`;
    msg += `- PAID PRICE: $${auction.paidPrice} uni/dai <br/>`;
    msg += `- UNI MED: $${auction.bidPrice} uni/dai <br/><br/>`;

    msg += 'Ended: <br/>';
    msg += `- DATE: ${auction.dealDate} <br/>`;
    msg += `- FROM: ${auction.txFrom} <br/>`;
    msg += `- PAID PRICE: $${auction.paidPrice} uni/dai <br/>`;
    msg += `- UNI MED: $${auction.dealPrice} uni/dai <br/><br/>`;
